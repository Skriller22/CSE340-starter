-- Database Schema for CSE340 Project
-- Name: cse340mjr
-- DB name: cse340mjr
...
-- Drop existing custom_order table if it exists (for rebuild)
DROP TABLE IF EXISTS public.custom_order CASCADE;

-- Create ORDER STATUS enum type
CREATE TYPE public.order_status AS ENUM ('Pending', 'In Review', 'Approved', 'Rejected', 'In Production', 'Completed');
ALTER TYPE public.order_status OWNER TO cse340mjr;
...
CREATE TABLE IF NOT EXISTS public.custom_order (
    ...
    order_make character varying NOT NULL,
    order_model character varying NOT NULL,
    order_year character(4) NOT NULL,
    order_color character varying NOT NULL,
    order_description text NOT NULL,
    order_estimated_price numeric(9, 0) NOT NULL,
...
-- Add index on account_id for faster queries when searching user's orders
CREATE INDEX idx_custom_order_account_id ON public.custom_order(account_id);

-- Add index on order_status for filtering by status
CREATE INDEX idx_custom_order_status ON public.custom_order(order_status);